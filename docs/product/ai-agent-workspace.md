...
- **写盘不受沙箱约束这句话要带一个前提**：`bash` 的**命令内容**不在硬沙箱的路径校验层里 —— 沙箱写面加固只作用于 `read` / `write` / `edit` 一族与临时目录 API。因此**本加固不封闭 `bash` 写盘**；但 `bash` **内部**用 `ln -s` 把工作区外的路径映射进来再写，会在这层校验里被拒（见 [§七](#七安全边界诚实声明) 的 SEC-10 条目）。
...
- **（第 5 条）安装档只审一级 bash 命令**：策略引擎看的是用户在卡片上看到的那条命令。若该命令内部再 `spawn` 子进程（如技能自带的 `check_env.mjs` 内部 `spawnSync` 调 `python3`），二次调用不在策略视野内 —— 由用户对第一条命令的确认承担。**「漏检 ≠ 免确认」成立，但要带前提**：由默认拒绝规则消除的，是「子命令词法改写」（`brew "install" wget`）、「中间 token」（`brew cask install`）与「未收录子命令」（`npm update` / `npm rebuild`）这三类**首 token 是包管理器的改写形态**；**剩余漏检类别不止一条**，其中「首 token 不是包管理器」（如变量间接 `NPM=npm $NPM i x`）这类命令**不以包管理器开头 → 天然不命中白名单前缀 → 仍退化为普通确认卡片（不会零卡片）**；但另有**三类**会因白名单前缀命中而**零卡片**，逐条列在下面的具名残余里。
...
- **（第 7 条）SEC-10 沙箱写面加固：一次行为变更（正确行为，但是变更）**。加固后 `write` / `edit` 一族的路径校验**不只看词法路径，还会复核「最近已存在祖先的 realpath」**。因此：
  - **`bash` 里「先 `ln -s <工作区外路径> link`，再往 `link` 写」会被拒**。这是**正确**行为（沙箱封闭），但相对加固前**是行为变更** —— 以前这种写法会真的写到工作区外。同一个判据也覆盖 `write` / `edit` / 临时目录 / 删除等**全部写面**。
  - **工作区内自指的 `ln -s` 仍放行**（realpath 归一化后仍在根内），因此「在工作区里建目录链接再写」这类正常用法不受影响。
  - **`exec` 的命令内容不在这一层校验** —— 加固只收紧「由本进程解析出的写目标」，**不**声称封闭 `bash` 写盘：`bash` 的边界仍然只有三档权限 + 确认卡片（见 §四）。换句话说，**能拦住的是「用链接把写操作骗过路径校验」，拦不住的是「用户确认过的命令里明写了一个工作区外路径」**。
  - **读面语义一字未改**：`readTextFile` 一族的行为与加固前完全一致。
