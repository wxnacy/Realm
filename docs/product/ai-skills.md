...
- 单元测试：`node tests/test-ai-skills.js`（**147 例，实测**）—— 加载管线 / 诊断 / 限额 / 启停 / prompt 注入 / Agent 回写 / P8 机制断言；覆盖已扩到**显式调用解析与实时读盘 / 面板投影收窄 / 三档 tier / `promptOmitted` / `read` 卡片标记 / 重载装饰 / 运行期新增技能的调用瞬间重扫 + 当场读盘（G-48-12） / 延迟补刷在纯文本流的成功出口落地（G-48-18） / 重扫或重试读盘抛错沿用原判定（G-48-19）**
