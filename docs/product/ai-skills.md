...
- **卡片头部**给出来源档位徽标（用户 / 内置 / 托管三档，与 `/` 面板**共用同一张查表**）与**至多一条**标注（失败短原因，或「未进提示词」）。两处刻意省略：`delete` 成功后**不显示**来源徽标（目标已消失，宁缺勿猜）；重开对话后的失败历史卡片**不显示**短原因（原因码不落库，只能如实呈现「失败」而无细节）。
...
**未闭合项如实挂账**：本节不声称已闭合 `TD-48-01` / `TD-48-02` / `WR-01` / `WR-02` / `WR-06`；也不声称失效链已完整闭合 —— 本阶段只补齐其中一段（AI 自建技能这一写入入口），技能集的另一个实际写入入口归 Phase 50 设置页，**合计约 1/3**。
