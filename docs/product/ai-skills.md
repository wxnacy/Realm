...
- 单元测试：`node tests/test-ai-skills.js`（**139 例，实测**）—— 加载管线 / 诊断 / 限额 / 启停 / prompt 注入 / Agent 回写 / P8 机制断言；覆盖已扩到**显式调用解析与实时读盘 / 面板投影收窄 / 三档 tier / `promptOmitted` / `read` 卡片标记 / 重载装饰 / 运行期新增技能的调用瞬间重扫 + 当场读盘（G-48-12）**
...
- （G-48-12）**成本**：每次「缓存未命中」的显式调用会多付一次全量重扫（与打开 `/` 面板同款：
  ...
  已缓存的技能不会多付这份代价。
- （G-48-12）**忙时语义**：显式调用路径**恒**处于「正在处理」（`prompt()` / `promptWithContext()`
  在解析技能**之前**就置位），因此那次重扫**不会**在轮内改写 system prompt、也**不**广播 ——
  只把「待回写」标记置脏；prompt 回写与 `skills:changed` 广播延后到下一次非忙同步点（打开面板 /
  Agent 重建 / 带附件或引用的那一轮结束）落地。这是刻意的：轮内改写对当前这一轮无效，还会让
  状态机更难推理。
- （G-48-12）**残余窗口**：重扫只发生在「缓存未命中」与既有触发点（打开面板 / Agent 创建或重建 /
  下一轮 idle 边界）上。因此以下两种**缓存命中**情形要等到下一次重扫才被纠正：① 同名技能在
  **更高优先级目录**运行期新增（重扫前仍调用旧胜出者）；② 禁用清单在无重扫期间被改动
  （46 D-09 的既有口径：禁用清单只在重扫时生效，本阶段不改变这一点）。
