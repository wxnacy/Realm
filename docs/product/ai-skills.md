...
- **技能正文折叠块的键盘语义只在气泡实例施加**：`role="button"` + `tabindex="0"` + `aria-expanded` + Enter / Space 只施加于**气泡实例**（该 header 在那里恒可见，控件可见则**焦点语义**成立）；**卡片实例**不施加 —— 原因是卡片的正文折叠块位于**默认折叠的容器内**（`.tool-card-content` 用 `max-height: 0` + `overflow: hidden` 承载折叠，而 `overflow: hidden` **不移出 Tab 序**），施加即产出一个**零可见高度**的、可 Tab 到达却看不见的停靠点，焦点环被祖先裁掉。卡片的展开 / 折叠（含卡片头部）保持全仓工具卡片的**鼠标语义**（`49-UI-SPEC.md` 的交互契约锁定「本阶段零改动」）。实现上是 `renderSkillContentBox` 的**语境开关**（`{ interactive = true }`，卡片调用点传 `false`）；这是**卡片形态**的说明，故落在 §11.7 而非 §11.6。
