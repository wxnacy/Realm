...
**录制挂钟时长 > 产物媒体时长**（如录 10 秒只得 6 秒）：两套口径差 = 启动
损耗（首轮基线等待 + 拉取延迟 + 轮询等待）。2026-09-09 已修：
`media-record-engine.js` 首轮从纯基线改为策略落盘（无 K 标记流直接落盘
最新分片；B 站 `EXT-BILI-AUX K` 标记流从最新 K 分片起录——同时根治
B 站产物头部 ~2s 花屏，那是 mid-GOP 起点引用录制前参考帧所致，与容器
无关），轮询间隔改 `min(targetDuration, EXTINF 中位数)`（Twitch 6s→2s）。
实测 B 站启动损耗 2.2s→1.0s、产物首帧即关键帧（零解码错误）。
