...
function createRecordEngine({ fetchPage, parsePlaylist, taskManager, recordRoot, maxConcurrent = 2, maxRetries = 5 }) {
  ...
  async function startRecord(input) {
    ...
    const st = {
      ...
      totalBytes: 0,
      startedPerf: performance.now(),  // IN-11：运行中时长锚点用单调时钟（免疫系统时钟跳变/校时），与分片落盘节奏解耦
      targetDuration: 6,
   ...
   * 运行中 durationSeconds 为单调时钟差值（startedPerf 起平滑推进，0 分片落盘也在走表，
   * G-44-4；IN-11 改 performance.now() 免疫系统时钟跳变）；与 meta.json totalDuration
   * （分片 EXTINF 累计）是两套口径，互不渗透。
   * @param {string} taskId - 任务 ID
  ...
  function getRecordStatus(taskId) {
    ...
    if (st) {
      return {
        ...
        durationSeconds: Math.max(0, Math.round((performance.now() - st.startedPerf) / 1000)),
      };
    }
   ...
   * 运行中 durationSeconds 与 getRecordStatus 同式：单调时钟差值（G-44-4 / IN-11），
