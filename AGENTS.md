...
**在 worktree 内跑测试（含需要 Electron 的端到端驱动）**：向上查找**只对裸说明符生效**。驱动里写 `require(path.join(REPO_ROOT, 'node_modules', 'electron'))` 或手拼 `.../node_modules/electron/dist/...` 会绕开复用机制，在 worktree 内直接 `MODULE_NOT_FOUND`（2026-09-15 实测；同一 worktree 下 `require('electron')` 能解析到主仓库的二进制，`require('better-sqlite3')` 同理）。因此：
...
- 测试：`node tests/test-agent-workspace.js`（沙箱/迁移 + **写面加固用例组**（`resolveInsideForWrite` 的逃逸转拒 / 自指链接放行 / 五个写方法与两个临时目录方法的切换 + 正反源码契约），38 例）、`node --test tests/test-ai-bash-policy.js`（策略引擎，97 例）、`node tests/test-builtin-skills-seeder.js`（播种 / 差异诊断 / 零安装语义 / 归属断言，新增）、`node --test tests/test-skill-picker-model.js`（面板纯逻辑：解析与 args 取值 / 过滤两档 / 展平与可选中性 / 导航取模 / 三条接线扫描 / 气泡构建单源与回填后定向刷新，115 例；含卡片标记并入函数 `mergeManageSkillMarker` 的值域与卡片头部超预算标注的 ≤ 4 字投影 / 48 原文冻结 / 九码长度上限）、`node --test tests/test-ai-cancel-state.js`（取消归属与用户气泡时序的纯逻辑用例 + renderer 接线护栏）、`node tests/test-manage-skill.js`（AI 自建技能域：`manage_skill` 三动作 / 校验器值域 / 四类撞名与 seeded 三入口保护 / 原子写与失败清理 / 字段分离扫描与扫描-净化顺序 / 数量闸 / 幽灵技能与越界护栏（含 description 值域与写↔读闸口边界：`: ` / `#` / 裸标量 / 纯零宽），55 例）、`node tests/test-ai-skills.js`（技能域全量单测：加载管线 / 诊断 / 限额 / 启停 / prompt 注入 / Agent 回写 / 显式调用解析与实时读盘 / 投影与 tier / `read` 卡片标记 / 重载装饰 / 运行期新增技能的一次性权威重扫 / 延迟补刷在纯文本流的成功出口落地（G-48-18） / 重扫或重试读盘抛错沿用原判定（G-48-19） / `manage_skill` 卡片标记两时点与终态元数据通道（M 组） / `manage_skill` 刷新链时序与次数账（L 组） / 失败态原因码的词缀与重载还原 / 三态 `promptIncluded` 消费侧 / a11y 增量的条件施加（M9b：守卫内施加 + 两调用点取值 + 单源未破），198 例）、`node tests/test-skills-management.js`（技能管理面：管理投影形状与三档分组 / 尺寸计量口径（含「统计不可用」分支与「不进 digest」的值副本）/ 启停语义 / 仅 user 可卸载的三态拒绝面与卸载后名单清理 / 管理面名称谓词与禁用名单双键校验 / 双入口转发层与注入纪律源码扫描 / 行内交互面与诊断两层承载，49 例）、`node tests/test-skills-http-api.js`（HTTP 与传输面：两个写子路由与转发目标 / `{ error, code }` 错误形状 / SEC-09 请求体体积闸的真实 http 行为（413 可达 / 无 unhandledRejection / 堆不线性增长 / 反向对照）与降级分支 / `readRawBody` 的四组 + 源码契约组（`maxBytes` 用 `MAX_SKILL_PACKAGE_BYTES`）/ 调用点覆盖度与常量单源 / 双入口跨文件一致性，41 例）、`node tests/test-skills-import.js`（用户技能导入面：zip 全量校验（两路 symlink / 落点 realpath / 六类限额 / NFD+小写归一化查重 / 逃逸族）/ 冲突三档 / 覆盖事务与回滚 / 回读验证失败即回滚 / 句柄生命周期（TTL / 并发上限 / 取消幂等 / 崩溃残留清扫）/ 拒绝面矩阵（21 码零跳过）/ 单点落盘源码扫描，116 例）、`node tests/test-skills-import-net.js`（用户技能导入**网络面**：URL 三形态分流 / 白名单精确匹配 / 逐跳私网 / 跳数上限抛错 / magic bytes / 流式字节上限 / 两来源共用同一条后段，**本地 stub server 不发外网**，50 例）
- **需要 Electron 的端到端驱动不在上列**：`tests/uat-*.js` 与 `tests/test-unified-navigation.js` 会**真实启动 dev 应用**，运行必须带全局 playwright —— `NODE_PATH="$(npm root -g)" node tests/uat-<name>.js`（`playwright` **只在全局**，仓内 `require.resolve('playwright')` 直接 `Cannot find module`；缺全局时驱动以 `E-PW` 码非零退出，属**环境前置缺失**而非断言失败）。此类驱动取 Electron 二进制**必须用裸 `require('electron')`**，拼 `path.join(ROOT, 'node_modules', ...)` 绝对路径会在 worktree 内 `MODULE_NOT_FOUND` —— 实测表、机械判据与已知待修清单见 [docs/dev/branching-spec.md](docs/dev/branching-spec.md) 第三节「在 worktree 内跑自动化测试」。
...
- **技能管理面（设置页 + `/api/skills/*`）的维护约定**：产品说明权威 = **[docs/product/ai-skills.md](docs/product/ai-skills.md) 第十二节**；改动**列表字段与口径 / 体积计量口径 / 启停与卸载语义 / 拒绝面与错误码 / 两入口的转发层 / 状态链与单源文案表 / `/api/*` 的请求体体积闸**时，**必须同步该章节与该节测试清单**。四条硬约束：① 判据 / 校验 / 投影全住 `ai-skills-manager.js`，该模块**零 electron 依赖** ⇒ 设置页的 HTTP 端点与主窗口的 IPC 通道**直接复用同一份**，handler 只做**转发**、零判定（这是「两入口同一权威」的全部机制）；② 管理面名称**谓词**与写入门（`validateManagedSkillName`）**故意不同宽**（禁用名单是消费侧信号）—— 不得「统一」成严格形态，否则会 render 出一个「点了必然失败」的开关；③ 写路径「重扫**恰一次** + 调用侧补播**恰一次**」，**不得改 `syncAgentSystemPrompt()` 的函数体**（46-04 的方法体断言与 48 的广播次数断言同时钉着它），补播**不得**挪进函数体；④ `LIMITS` **只允许加项**、前端**零字面量**（限额数值一律取管理投影回传的 `limits`）。**测试口径**：`node tests/test-skills-management.js`（管理数据面）+ `node tests/test-skills-http-api.js`（HTTP / 传输面）；两者用例的 `seededNames` 必须**显式注入**（纯 Node 下 `getSeededSkillNamesSafe()` 会降级为 `[]`，会让 seeded 保护用例假绿）；**禁止 `npm test`**（本项目没有该脚本，两处 gate 恒把它解析成不存在的脚本）。**诚实边界成文**：设置页收不到主进程广播 ⇒ 多开设置页之间不做即时同步（详见该章节的诚实边界小节）。
- **用户技能导入（zip + 网络地址）的维护约定**：产品说明权威 = **[docs/product/ai-skills.md](docs/product/ai-skills.md) 第十三节**；改动**限额常量（`IMPORT_LIMITS` / `LIMITS`）/ 错误码表（`IMPORT_SKILL_ERROR`）/ 威胁模式表（`SKILL_THREAT_PATTERNS`）/ 主机白名单（`HOST_WHITELIST`）/ 技能根定位口径 / 冲突与覆盖语义 / 落点边界（`importId` 不透明）/ `/api/skills/import` 的 body 分流与体积上限**时，**必须同步该章节与该节测试清单**。四条硬约束：① 判定 / 校验 / 解压 / 落盘**全住 `ai-skills-manager.js`**（该模块**零 electron 依赖**）⇒ `main.js` 的 handler 与 `ai-manager.js` 只做转发，**落盘实现全仓只有一个调用点**（源码扫描判据）；② **三条来源共用一条解压入口**（本地上传 / zipball / raw 单文件都先落到同一个临时包区，再进 `extractAndValidatePackage` 或 `prepareFromRawFile`，之后是**同一段**后段）；③ **`LIMITS` 只允许加项**、导入限定值一律住 `IMPORT_LIMITS`、**端点与前端零字面量**（限额数值只经 preview 响应的 `limits` 回传）；④ **覆盖的临时备份不得扩张成版本历史**（成功后即刻删除，不提供「撤销上一次导入」）。另加一条**码表隔离**：导入码住 `IMPORT_SKILL_ERROR`，**不得并入** `MANAGE_SKILL_ERROR`（后者被 `tests/test-manage-skill.js` 锁死为**恰 11 键**）。**四条不变式（改动时必须同步）**：① **写侧权威字节闸口的判据对象 = 组装后的 `SKILL.md` 全文**；导入路径**不重写文件** ⇒ 导入侧的判据对象是**包内 `SKILL.md` 的实际字节数**（这条口径差必须在文档里说清，否则会有人「顺手」改成按正文长度判）；② **上传体积闸对 zip 炸弹零贡献** —— 正确表述是「合法包的解压总量 ≤ 32 MiB ⇒ 其压缩后体积必然 ≤ 32 MiB，故上传闸不会误杀合法包」；**压缩比闸与累计解压闸是两道独立的自建闸**；③ **嵌套深度按「定位到的技能根」相对计**（**技能根相对**计，不是包根 —— 包根口径下 `anthropics/skills` 的 `docx` 恰好 depth 8，会把官方技能打满）；④ **覆盖的临时备份不是版本历史**。**诚实边界五条**（指向第十三节 13.10 与 `ai-agent-workspace.md` §七）：威胁扫描三类是启发式、**不是安全边界**；DNS rebinding 残余风险（被校验的 IP ≠ 被连接的 IP，**未消除**，白名单只把收益压到接近零）；SEC-10 的行为变更；`allowed-tools` 当前运行时不被强制；白名单里 `skills.sh` / `www.skills.sh` / `objects.githubusercontent.com` 三条**保留为未来兼容、本阶段无消费者**（**不得**沿用「承载产品闭环」这条已被实测推翻的理由）。**本阶段踩过的坑（供后来者复用，全部为本阶段实测）**：① `yauzl` 的 `eachEntry()` 会 close 整个 zipfile ⇒ **开流必须在 `for await` 体内**；② central directory 的 `validateFileName` 会**放行六类**逃逸形态（须自行补判据）；③ `uncompressedSize === 0xFFFFFFFF`（zip64 声明）会**静默通过**，须显式拒绝为 `unsupported_zip64`；④ `yaml` 必须**精确钉 `2.9.0`**（写 `^2.9.0` 会与 SDK 的精确钉版共产出**双实例**，解析差异是静默的）；⑤ 威胁模式的「诱导跳过确认」类必须带**前置否定词**（负向先行断言 —— **lookbehind 实测无效**），否则会命中内置 `find-skills` 自身。
