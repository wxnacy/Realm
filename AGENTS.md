...
- 测试：`node tests/test-agent-workspace.js`（沙箱/迁移，21 例）、`node --test tests/test-ai-bash-policy.js`（策略引擎，97 例）、`node tests/test-builtin-skills-seeder.js`（播种 / 差异诊断 / 零安装语义 / 归属断言，新增）、`node --test tests/test-skill-picker-model.js`（面板纯逻辑：解析与 args 取值 / 过滤两档 / 展平与可选中性 / 导航取模 / 三条接线扫描）
