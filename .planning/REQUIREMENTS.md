...
- [x] **DISC-05**: 模型可根据 description 自动匹配技能并读取其正文（经 `read` 工具读取 SKILL.md 的 location）
...
- [x] **USER-03**: 用户可通过上传 **zip 包**导入技能（单技能包语义；0 个或多个技能根 → 报错并提示）
- [x] **USER-04**: 用户可通过**网络地址**导入技能，自动分流 GitHub 仓库/目录地址 与 SKILL.md 直链
- [x] **USER-05**: 导入采用两阶段（预览 → 确认落盘），预览展示名称 / description 原文 / 目录树 / 字节数 / 脚本清单（标红）/ 威胁扫描结论
...
- [x] **USER-08**: 导入失败给出真实原因（命中哪个限额 / 扫描结论 / 校验错误），不静默
...
- [x] **SEC-02**: zip 解压**拒绝含 symlink entry 的整包**（读 central directory 属性判定 + 解压后递归 `lstat` 复核整棵树），而非跳过单条
- [x] **SEC-03**: 逐 entry 路径校验：`..` 段（`path.posix.normalize` 后判）/ 绝对路径 / 盘符 / UNC / 反斜杠 / NTFS ADS / 控制字符 / 尾随空格与点；并对全量 entry 名做 NFD+小写归一化查重（大小写与 Unicode 冲突即拒绝整包）
- [x] **SEC-04**: 解压先读 central directory 的 `uncompressedSize` 预检，再边解边累加；限额覆盖单 entry 字节、累计字节、entry 数、压缩比、嵌套深度
- [x] **SEC-05**: 解压到 `fs.mkdtempSync` 创建的全新空目录（保证前缀内不可能有预埋链接）；落点复核使用**最近已存在祖先的 realpath**，而非仅依赖 `resolveInside`
- [x] **SEC-06**: 导入时同时扫描 `description` 与 body：复用 `scanInjectionPatterns`，并新增技能域 `SKILL_THREAT_PATTERNS`（文件外发 / 凭据回显 / 诱导跳过确认三类）
- [x] **SEC-07**: 与内置技能同名 → 拒绝导入；与已有用户技能同名 → 显式策略（覆盖 / 改名 / 取消），不静默覆盖
- [x] **SEC-08**: 网络导入 https-only + 主机白名单 + 逐跳 `isPrivateHost` 校验 + 流式字节上限 + magic bytes 校验；**不复用** `search-manager.fetchUrl`（它会把内容转 Markdown，对二进制是破坏性的），复用其 SSRF 判据
- [x] **SEC-09**: `readJsonBody` 增加体积上限（防 zip base64 放大无上限）
- [x] **SEC-10**: 加固既有沙箱 `writeFile` 的 ENOENT symlink 缺口（与导入路径同一根因，一次修完）
...
| USER-03 | Phase 51 | Complete |
| USER-04 | Phase 51 | Complete |
| USER-05 | Phase 51 | Complete |
| USER-06 | Phase 50 | Complete |
| USER-07 | Phase 50 | Complete |
| USER-08 | Phase 51 | Complete |
...
| SEC-02 | Phase 51 | Complete |
| SEC-03 | Phase 51 | Complete |
| SEC-04 | Phase 51 | Complete |
| SEC-05 | Phase 51 | Complete |
| SEC-06 | Phase 51 | Complete |
| SEC-07 | Phase 51 | Complete |
| SEC-08 | Phase 51 | Complete |
| SEC-09 | Phase 50 | Complete |
| SEC-10 | Phase 51 | Complete |
