...
- [Phase 49]: [49-08] a11y 类判据的**命中测试只接受「自身或后代」**（`hit === el || el.contains(hit)`），不接受祖先 —— 计划原文允许祖先的形态在未修复树上即判绿（被 `overflow: hidden` 裁掉的后代，其中心点真正命中的正是承载裁切的祖先）。按 R6 纪律**先修驱动、不动源码**
- [Phase 49]: [49-08] 判据必须写**方向无关**而非「卡片内零键盘停靠点」—— 若日后真做全仓卡片范式升级（卡片头部成为可聚焦入口）那是 a11y **改善**，不应让门禁转红。真实不变式 = 「凡在卡片内的停靠点都必须可见」
...
- ⚠️ [Phase 49 · 收尾带出 · 可复现性债] **`IN-16` / `WR-09`**：两处真实渲染门禁（`uat-49-g49-3-panel-layout.js` 的 A9 红轮基准、`uat-49-g49-4-card-a11y-tab-order.js` 的红→绿证据）只活在 `/tmp/uat49/`，且 `E-DATA` / `E-DATA-DB` 前置依赖本机 `realm-dev` 的会话库与 `managed-skills` 目录 ⇒ 新机器 / CI 跑不起来。**结论不可重跑复核**
- ⚠️ [Phase 49 · 收尾带出 · 文案口径] **`IN-10` ~ `IN-13`**：文档与注释的口径滞后（含 `src/styles/main.css` 实测口径注释段把「祖先 scrollWidth」与「外接矩形口径」写成一个等式）。不阻断目标，见 `49-REVIEW.md`
- ⚠️ [Phase 48 · 继续挂账，**不得**读成已修] **WR-02 / WR-06**（用户裁决）：WR-02 = `ai-manager.js` 两处 `_resolveSkillInvocation` 裸调 + 首次读盘 + 动态 import 未包 `try`（异常逃逸时 `isProcessing` 可能不复位，48-08 只纳入了**新增的那个**调用点）；WR-06 = **已缓存**技能的实时读盘命中路径仍绕过 64 KiB 字节闸（`docs/product/ai-skills.md` §10.7 明文「仍开放」）。另 WR-01 / WR-03 / WR-04 与 IN-01~IN-17 仍在台账（IN-16 是唯一有潜在翻转风险的：若 `refreshSkills()` 日后改成「整批失败即抛错」，`if (rescanned)` 的跳过会把已刷新成功的技能判成 `not_found`）。全量见 `48-REVIEW.md`
