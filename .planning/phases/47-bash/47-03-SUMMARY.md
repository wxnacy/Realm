...
- `plan_head_before` = `f28cc2f93cfc71d6d6cc3c15a057c444d8b151ff`（来自 .git/gsd-plan-head-before-47-03 台账）。frontmatter 的 `commits: 4` 是**写 SUMMARY 时**的实测值（`git rev-list --count BASE..HEAD` = 4 个任务提交），符合 #3968「实测而非叙述」的要求；本计划的最终提交总数为 **6**（+ 元数据提交 `0456a7a` + 本自查提交），差额来自 SUMMARY 之后的收尾提交，**不要**据此判定 frontmatter 记错。
