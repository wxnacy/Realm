...
- `plan_head_before` = `f28cc2f93cfc71d6d6cc3c15a057c444d8b151ff`（来自 .git/gsd-plan-head-before-47-03 台账）。frontmatter 的 `commits: 4` 是**写 SUMMARY 时**的实测值（`git rev-list --count BASE..HEAD` = 4 个任务提交），符合 #3968「实测而非叙述」的要求。本计划的**任务提交固定为 4 个**（`a90e549` / `b69f57d` / `ca3aac8` / `90e090b`），其后另有若干**收尾提交**（元数据 `0456a7a`、本自查、以及对本行的措辞微调）；完整数量请以 `git rev-list --count <plan_head_before>..HEAD` 实测为准，**不要**把收尾提交计入 frontmatter 的 `commits` 口径。
