...
| 32 MiB 上传在本机 Electron 的真实耗时与内存峰值 | （D-01） | 研究在真实 Chromium 下测得 26 ms / 堆持平，但绝对值需本机复核 | **已自动化（`/gsd-verify-work 51` 新增驱动）** —— `node tests/uat-51-import-limits.js` 实测 **19/19 全过**。近限包（解压总量 31,744,134 B / 32 条目，**在 guest 内现场生成、不经 CDP 传字节**）上传 **906～1059 ms** 就绪；主进程 RSS 增量 ≈90.7 MiB（正命题：真的过了线）；确定性源码判据（剥注释后）断言导入区与 `readRawBody` 零 `arrayBuffer`/`FileReader` 且 `body` 逐字是 File 直传（含三条**单点变异自证**）；64 MiB 超限包 ⇒ `请求体超过上限（33554432 字节）`（含真实限额数字、非静默失败）。⚠️ **两条环境读数只登记不断言**：① guest JS 堆曲线 —— `performance.memory` 量化且与 GC 强耦合，显式 `arrayBuffer()` 的标定增量实测为**负值**；② **B 侧进程 RSS 上界** —— 初版的「RSS ≤ `maxBytes` + 8 MiB」经 verifier 复跑证明**分支相关且不稳定**（快路径 49 KB / 累积分支 44.4～68.1 MB，4 次红 2～3 次；排空瞬时 chunk 缓冲叠加 GC 滞后可让 RSS 峰值超过 body 体积本身），已删阈值断言、改记 `overRssReading` + 分支标签。「不无上限读入内存」改由 **`readRawBody` 的源码契约（`Content-Length` 预检 + 累积中判 + `req.resume()`）+ 三条单点变异自证**承重。修复后连跑 3 轮均 **19/19**，无抖动 | dev 模式打开 DevTools Performance，上传一个接近 32 MiB 的包，记录耗时与堆曲线；确认无「先 arrayBuffer() 再判大小」的峰值 |
