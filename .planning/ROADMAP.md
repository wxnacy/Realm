...
- [x] **Phase 51: 用户技能导入管线（zip + 网络地址）** - 两阶段预览 + 恶意包整包拒绝 + 网络导入 SSRF 防护 (completed 2026-09-15)
...
- [x] **51-02-PLAN.md — 两个依赖按实测口径落定（含 blocking-human 检查点）**：`yauzl@^3.4.0`（库自身从不写盘）+ `yaml@2.9.0`（**精确钉版** ⇒ 单实例）+ 依赖审计四条 + `build.files` 的 `!` 前缀护栏（USER-03、USER-04）
