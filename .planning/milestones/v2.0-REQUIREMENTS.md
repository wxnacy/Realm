...
**验收标准**:
- [x] 用户可以在收藏夹页面通过右键菜单或按钮创建新文件夹
- [x] 用户可以设置文件夹名称
- [x] 文件夹支持无限制层级嵌套（类似 Chrome）
- [x] 新创建的文件夹默认在当前目录下
...
**验收标准**:
- [x] 用户可以通过右键菜单选择"删除"
- [x] 删除前显示确认对话框，提示将删除所有子项
- [x] 删除文件夹时，其下的所有收藏项和子文件夹一并删除（级联删除）
- [x] 删除后自动导航到父文件夹
...
**验收标准**:
- [x] 用户可以通过拖拽将收藏项移动到文件夹
- [x] 用户可以通过右键菜单选择"移动到..."选择目标文件夹
- [x] 支持批量移动（选中多个收藏项后移动）
- [x] 移动后收藏项在原位置消失，出现在目标文件夹中
...
**验收标准**:
- [x] 可以拖拽收藏项调整在同一文件夹内的顺序
- [x] 可以拖拽文件夹调整顺序
- [x] 拖拽到文件夹上时高亮显示，松开后移入该文件夹
- [x] 排序结果持久化保存
...
**验收标准**:
- [x] 用户可以选择本地 HTML 书签文件导入
- [x] 支持标准的 Netscape Bookmark 格式
- [x] 保留文件夹结构
- [x] 导入时显示预览，用户确认后执行
...
- [x] 收藏栏位于地址栏和标签栏之间
- [x] 默认显示收藏夹根目录的内容
- [x] 显示收藏项的 favicon 和标题
- [x] 文件夹显示文件夹图标和名称
- [x] 收藏栏高度固定，内容超出时显示 ">>" 溢出按钮
...
**验收标准**:
- [x] 点击收藏项：在当前标签页导航到该 URL
- [x] Cmd/Ctrl + 点击收藏项：在新标签页打开
- [x] 点击文件夹：展开显示文件夹内容（下拉菜单）
- [x] 鼠标悬停在文件夹上：自动展开（延迟 300ms）
...
- [x] 如不满足，实施方案（按优先级）：（**留到后续 Phase 评估**）
