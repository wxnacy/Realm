...
该补齐作用于**唯一构建实现**（`renderSkillContentBox`）并由**语境开关**（`{ interactive = true }`）
控制 —— **气泡实例施加、卡片实例不施加**，因此不是无条件的纯增量：对**气泡实例**是纯增量
（零视觉变化、零文案变化、零行为回归），对**卡片实例**则必须关闭。

**为什么卡片实例必须不施加（`UI-49-W6-01` 的成因与修法 C）**：卡片把本构建产物放进
`.tool-card-content`，该容器用 `max-height: 0; overflow: hidden` 承载默认折叠（为保住
**200ms 的 `max-height` 过渡**）。`overflow: hidden` **不会**把后代移出**顺序焦点导航**
（只有 `display: none` 会）⇒ 施加焦点语义会产出一个**零可见高度**的 Tab 停靠点：焦点环被
祖先裁掉、命中测试取不到它，即「键盘焦点停在一处看不见的控件上」（WCAG 2.4.7 不达标）。
所以卡片调用点（`src/renderer.js` 的 `renderSkillContentBox({ content: params.content },
{ interactive: false })`）关闭该开关。

**为什么不用 `display: none` 替代**：那会一并杀掉 200ms 的展开 / 折叠过渡，爆炸半径覆盖
**全部**工具卡片 —— 等于用一个视觉回归换一个 a11y 修复；`visibility: hidden` 的等价方案
同样要动过渡与全部卡片，一并排除。

**为什么这不是范式分裂**：卡片的展开 / 折叠（含卡片头部）沿用全仓既有的**鼠标语义**，
即下方 `## 交互契约` 的「展开 / 折叠（卡片）」行所锁定的**本阶段零改动**；本修法正是为
**不推翻**那条锁定决策而选（把焦点语义上移到卡片头部会推翻它，故不取）。
...
| 展开 / 折叠（正文块） | 点击折叠块 header 切换 `.collapsed`；**新增** `role="button"` + `tabindex="0"` + `aria-expanded` + Enter/Space（**气泡实例**；**卡片实例**不施加 —— 卡片实例沿用与卡片头部同为鼠标语义的既有范式，理由见 `## 卡片结构契约` 的「键盘可达性」段与 `UI-49-W6-01`）。默认折叠、状态不持久化 |
