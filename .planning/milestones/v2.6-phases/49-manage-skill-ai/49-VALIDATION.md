...
|----------|-------------|------------|-------------------|
| 端到端可见性：AI 建技能后，**下一条消息**的应答中能引用该技能，工具卡片显示「创建技能「name」」+ 来源徽标 | MGMT-01 / MGMT-05（判据 1） | 需真实 LLM 往返与视觉观感，无法自动化裁决 | 见下方**五步逐条勾选**（每步均给出可判定的**期望现象**） |

**前置**：`npm run dev`（开发环境；`userData` 为 `realm-dev`，技能目录可手工清空以便从空集开始）。

**五步（逐条勾选，每条以「期望现象」为判据，不以「感觉正常」为判据）**

- [ ] **步骤①** 在 AI 聊天里**显式要求**它把某套流程沉淀为技能（例如：「把我们刚才确认的提交信息规范记成你的技能，名字用 commit-style」）。
  - **期望①**：出现一张 `manage_skill` 卡片，标题为「创建技能「commit-style」」，**运行中即可见**；完成后头部出现**托管**徽标；展开区看到参数摘要三行、默认折叠的「技能正文（N 字符）」折叠块、结果文本含「**下一条消息**起可用」。
- [ ] **步骤②** **不重开对话**，直接发下一条消息问「你现在有哪些技能？」。
  - **期望②**：应答中出现 `commit-style`（这是判据 1「下一条消息即对模型可见」的端到端证据 —— 断言点在**下一条消息**，**不是**工具返回时）。
- [ ] **步骤③** 手工在 `agent-workspace/managed-skills/` 下造一个与随包内置技能**同名**的目录（例如 `find-skills/`），再让 AI 对同名技能执行 create / update / delete。
...
- [ ] **步骤⑤**（backstop 的视觉确认）把 AI 面板拖到最小宽度（对应 `--ai-panel-min-width: 280px`），让一张卡片同时带来源徽标 + 「未进提示词 · 超预算」标注（若不便复现可用一个**超长技能名**）。
  - **期望⑤**：头部保持**单行**不换行、徽标与短原因完整可读、头部高度不变；**唯一允许的退化**是技能名被压缩到**省略号**。若不成立，处理方式是**缩短短原因至 ≤ 4 字** —— **不得**改成换行头部或加 system-note（UI-SPEC 明文）。

**证据分层声明（如实标注，不夸大）**

- **层 1（自动化，已在 49-01 / 49-02 落地）**：脏标记流转 / 成功出口回写 / 广播**恰一次** / `rescanCalls === 2` / 卡片标记两时点与重载同形 / 九码拒绝面与三条护栏。判定入口见上表 `Automated Command` 列与本文件 §Per-Task Verification Map。
- **层 2（仅人工，不可自动化）**：① 模型是否真的在**下一条消息**里能引用新技能（需真实 LLM 往返）；② 卡片在最小宽度下的可读性（**视觉裁决**）。二者都**不得**由层 1 的证据替代。
- **边界声明**：步骤③ 的「同名用户技能优先」一类拒绝若不便手工复现，可用 `node tests/test-manage-skill.js` 的对应用例作为证据（写入本表 Instructions 列即可），**不因人工不便而复述成「已人工验证」**。

**本任务未执行人工观察**：步骤与期望现象已就绪，实际执行属 `/gsd:verify-work` 的人工动作（本计划**不**创建 `49-UAT.md`、**不**改 `STATE.md`、**不**跑 `npm run dev`）。
